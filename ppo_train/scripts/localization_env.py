...
class LocalizationEnv(gym.Env):
    ...
    def ComputeReward(self, plicp_x_w_, plicp_y_w_):
        reward = 0.0
        # print("aaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaa")
        # print("aaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaa")
        for i in range(0, len(self.point_real)):
            if (i == 0):
                plicp_w = plicp_x_w_
            else:
                plicp_w = plicp_y_w_

            predict_ = self.point_orb[i][self.current_step+1] * (1-plicp_w) + self.point_plicp[i][self.current_step+1] * (plicp_w)

            ## situation 1:
            # real position is between orb and plicpa
            if ((self.point_real[i][self.current_step+1] <= self.point_orb[i][self.current_step+1] and \
                 self.point_real[i][self.current_step+1] >= self.point_plicp[i][self.current_step+1]) or \
                (self.point_real[i][self.current_step+1] >= self.point_orb[i][self.current_step+1] and \
                 self.point_real[i][self.current_step+1] <= self.point_plicp[i][self.current_step+1])):

                orb_dis   = abs(self.point_real[i][self.current_step+1] - self.point_orb[i][self.current_step+1])
                plicp_dis = abs(self.point_real[i][self.current_step+1] - self.point_plicp[i][self.current_step+1])
                between_range = abs( self.point_orb[i][self.current_step+1] - self.point_plicp[i][self.current_step+1])
                ...
                if ( orb_dis >= between_range/4 and \
                    ...
                    if (abs(predict_ - self.point_real[i][self.current_step+1]) <= between_range/4):
                        reward += 1 - (abs(predict_ - self.point_real[i][self.current_step+1]) / (between_range/4))
                    else:
                        reward -= (abs(predict_ - self.point_real[i][self.current_step+1]) - (between_range/4)) / (fartest - between_range/4)
                else :
                    if (abs(predict_ - self.point_real[i][self.current_step+1]) <= (fartest - between_range/2)):
                        reward += 1 -(abs(predict_ - self.point_real[i][self.current_step+1]) / (between_range/2 - shortest))
                    else:
                        reward -= ((abs(predict_ - self.point_real[i][self.current_step+1]) - (between_range/2 - shortest) ) / (between_range/2))

            ## situation 2:
            # real position isn't between orb and plicp
            elif ((self.point_real[i][self.current_step+1] > self.point_orb[i][self.current_step+1] and \
                   self.point_real[i][self.current_step+1] > self.point_plicp[i][self.current_step+1]) or \
                  (self.point_real[i][self.current_step+1] < self.point_orb[i][self.current_step+1] and \
                   self.point_real[i][self.current_step+1] < self.point_plicp[i][self.current_step+1])):
                ...
                if ((abs(predict_ - self.point_real[i][self.current_step+1]) - shortest) < (between_range/2)):
                    reward += ((abs(predict_ - self.point_real[i][self.current_step+1]) - shortest) / (between_range/2))
                else:
                    reward -= ((abs(predict_ - self.point_real[i][self.current_step+1]) - shortest - (between_range/2)) / (between_range/2))
        return reward
    ...
    def step(self, action):
        ...
        reward = self.ComputeReward(plicp_x_w, plicp_y_w)
    ...
    def SaveLog(self, reward, action):
        ...
        self.log_ORB_MSE_x = self.ComputeMSE(self.point_real[0][self.current_step+1]  , self.point_orb[0][self.current_step+1])
        self.log_ORB_MSE_y = self.ComputeMSE(self.point_real[1][self.current_step+1]  , self.point_orb[1][self.current_step+1])
        self.log_PLICP_MSE_x = self.ComputeMSE(self.point_real[0][self.current_step+1], self.point_plicp[0][self.current_step+1])
        self.log_PLICP_MSE_y = self.ComputeMSE(self.point_real[1][self.current_step+1], self.point_plicp[1][self.current_step+1])

        our_x = self.point_orb[0][self.current_step+1] * (1-action[0]) + self.point_plicp[0][self.current_step+1] * (action[0])
        our_y = self.point_orb[1][self.current_step+1] * (1-action[1]) + self.point_plicp[1][self.current_step+1] * (action[1])

        self.log_OUR_MSE_x = self.ComputeMSE(self.point_real[0][self.current_step+1], our_x)
        self.log_OUR_MSE_y = self.ComputeMSE(self.point_real[1][self.current_step+1], our_y)
